...
const addressRequests = {
    add:{
        ...
        house_building_no: Joi.string().min(1).max(30).required().label("House/Building No."),
    ...
    edit:{
        ...
        house_building_no: Joi.string().min(1).max(30).empty(["",null]).label("House/Building No."),
        address: Joi.string().min(3).max(100).empty(["",null]).label("Address Line"),
        ...
        lat:Joi.string().label("Lattitude").empty(["",null]),
        long:Joi.string().label("Longitude").empty(["",null]),
    }
}
