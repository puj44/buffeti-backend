...
async function OrderConfirmationSms(name, order_number, mobile) {
  try {
    // PLACE ORDER SMS
    const msg = `Dear ${name}, your order (ID: ${order_number}) has been accepted. Thank you for ordering with us! GNV CLICK2CATER.`;
    //TODO REMOVE return
    return true;
    const res = await axios.get(
      `http://13.234.156.238/api.php?username=gnvctc&apikey=${process.env.SMS_API_KEY}&senderid=${process.env.SMS_SENDER_ID}&route=TRANS&mobile=${mobile}&text=${msg}`
    ...
    console.log("PLACE CONFIRMATION SMS ERROR:", res?.status, res?.data ?? res);
    return false;
  } catch (err) {
    console.log("PLACE CONFIRMATION SMS ERROR:", err);
    return false;
  }
}

module.exports = { OtpRequest, OrderPlacedSms, OrderConfirmationSms };
