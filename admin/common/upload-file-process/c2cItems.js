    ...
    if (jsonData?.length <= 0) {
      ...
      jsonData.slice(1).map((row, rowIndex) => {
        if (row?.length > 0) {
          if (
            row?.[indexes.item] &&
            row?.[indexes.item]?.toString()?.trim() !== ""
          ) {
            ...
            itemObj = {
              ...
              sub_category: row[indexes.sub_category]?.toString()?.trim()
                ? {
                    slug: convertToSlug(
                      row[indexes.sub_category]?.toString()?.trim()
                    ),
                    name: row[indexes.sub_category]?.toString()?.trim(),
                  }
                : {},
              rate_per_serving: Number(row[indexes.rate_per_serving] ?? 0),
              is_additonal_serving: row[indexes.additional_serving]
                ? true
                : false,
              additional_serving: row[indexes.additional_serving]
                ? Number(row[indexes.additional_serving])
                : null,
              additional_serving_unit: row[indexes.additional_serving_unit]
                ? row[indexes.additional_serving_unit]
                    .toString()
                    .trim()
                    .toLowerCase()
                : null,
              additional_serving_rate: row[indexes.additional_serving_rate]
                ? Number(row[indexes.additional_serving_rate])
                : null,
              food_cost: row[indexes.food_cost]
                ? Number(row[indexes.food_cost])
                : null,
            ...
            //SAVE CATEGORY
            if (
              category?.toLowerCase() !== "extra-item" &&
              category.toLowerCase() !== "preparation"
            ) {
              let sub_cat = {};
              ...
              if (
                row[indexes.extra_items] &&
                row[indexes.extra_items].toString().trim() !== ""
              ) {
              ...
              if (
                row[indexes.preparation] &&
                row[indexes.preparation].toString().trim() !== ""
              ) {
                //COMMA SEPERATE INTO ARRAY
            ...
            throw Error(
              "Package Menu - Problem at row number: " + rowIndex + 1
            );
          }
        }
      });
