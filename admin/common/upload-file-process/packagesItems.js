  ...
  try {
    ...
    if (jsonData?.length <= 0) {
      ...
      jsonData.slice(1).map((row, rowIndex) => {
        if (row?.length > 0) {
          if (
            row?.[c2cIndexes.package] &&
            row?.[c2cIndexes.package]?.toString()?.trim() !== ""
          ) {
            ...
            const briefDescription = row[c2cIndexes.categories]
              ?.toString()
              ?.trim();
            ...
            splitCategories.forEach((cat) => {
              ...
              categoriesMapping[categorySlug] = Number(
                splitQty[0]?.toString()?.trim()
              );
            });
            ...
            globalObj.push({
              ...
              category: {
                slug: row?.[c2cIndexes.category]
                  ? convertToSlug(row?.[c2cIndexes.category])
                  : null,
                name: row?.[c2cIndexes.category] ?? null,
              },
              rate: Number(row[c2cIndexes.rate]?.toString()?.trim()),
            });
          } else {
