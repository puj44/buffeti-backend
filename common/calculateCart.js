    ...
    if(cartData?.coupon_code && cartData?.coupon_code !== null){
      ...
      coupon_discount = couponData.discount_value
        ...
        if(couponData.discount_type === "PCT"){
          couponVal =  (billingDetails.total_amount * couponData.discount_value)/ 100;
        }else{
...
const addCouponCodeCache = async(customerId, code) =>{
  try{
    ...
    cartData.billing_details = await calculateFromItemsAmount(billingDetails,cartData);
...
const deleteCartItemCache = async (customerId, packageName) =>{
  try{
    let cartData = await get(`cart-${customerId}`,true);
    const reducePrice = Number((cartData.cart_data?.[packageName]?.price ?? 0) * (cartData.cart_data?.[packageName]?.no_of_people ?? 0));
    let billingDetails = cartData.billing_details;
    billingDetails.total_items_amount = Number(billingDetails.total_items_amount - reducePrice);
